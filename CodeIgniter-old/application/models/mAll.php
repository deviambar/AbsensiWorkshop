...
class mAll extends CI_MODEL{
    ...
    function showDataAbsensi(){
        ...
        $this->db->join('tbl_siswa', 'tbl_pendaftaran.id_siswa = tbl_siswa.id_siswa');
        $this->db->join('tbl_event', 'tbl_pendaftaran.id_event = tbl_event.id_event');
        $this->db->join('tbl_jurusan', 'tbl_siswa.id_jurusan = tbl_jurusan.id_jurusan');
        $this->db->group_by("nama_event");
        $query = $this->db->get();
        return $query->result();   
    }
