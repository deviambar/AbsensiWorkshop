...
class mUser extends CI_MODEL{
    ...
    function check_date($tgl,$jam,$wId_event){
        date_default_timezone_set('Asia/Jakarta');
        $this->db->select('*');
        $this->db->from('tbl_event');
        $this->db->where($wId_event);
        $this->db->where('tanggal_event =','$tgl');
        $this->db->where('jam_mulai_event >= ','$jam');
        $query = $this->db->get();
        return $query->result();
    }
    function check_already_registered($id,$wId_event){
        $this->db->select('*');
        $this->db->from('tbl_pendaftaran');
        $this->db->where('id_event =','$wId_event');
        $this->db->where('id_siswa =','$id');
        $query = $this->db->get();
        return $query->result();
    }
    function check_file($where){
        $this->db->select('*'); 
        $this->db->from('tbl_pendaftaran');
        $this->db->where($where);
        $query = $this->db->get();
