...
class Home extends CI_Controller {
    ...
    public function check_file_exist(){
        ...
        $check = $this->mUser->check_file($where);
        $count = count($check);
        if (empty($count)){
	...
	public function daftar_action(){
        $id = $this->input->post('id');
        $id_event = $this->input->post('id_event');
        $check = $this->mUser->check_already_registered($id, $id_event);
        if(count($check) > 0){
            echo"
            <script>
    ...
    public function check_in(){
        ...
        $check = $this->mUser->check_date($tgl,$jam,$wId_event) ;
        if($check > 0){
            $where = array(
